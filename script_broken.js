...
function processFrame() {
    ...
    try {
        ...
        // Verbesserte Bildverarbeitung v1.1
        cv.cvtColor(scaled, gray, cv.COLOR_RGBA2GRAY);
        
        // 1. Bilateral Filter für Rauschreduzierung bei Kantenerhaltung
        let filtered = new cv.Mat();
        cv.bilateralFilter(gray, filtered, 9, 75, 75);
        
        // 2. Kontrast-adaptive Parameter
        let mean = new cv.Scalar(), stddev = new cv.Scalar();
        cv.meanStdDev(filtered, mean, stddev);
        const contrast = stddev.val[0];
        const brightness = mean.val[0];
        
        // 3. Adaptive Canny Threshold basierend auf Bildqualität
        const sigma = 0.33;
        const median = calculateMedian(filtered);
        const lower = Math.max(0, (1.0 - sigma) * median);
        const upper = Math.min(255, (1.0 + sigma) * median);
        
        console.log(`v1.1 Adaptive Thresholds: lower=${lower.toFixed(1)}, upper=${upper.toFixed(1)}, contrast=${contrast.toFixed(1)}`);
        
        // 4. Morphologische Operationen für Rauschunterdrückung
        ...
        // 6. Post-Processing: Schließe kleine Lücken
        let dilated = new cv.Mat();
        cv.dilate(thresh, dilated, kernel, new cv.Point(-1, -1), 1);
        cv.erode(dilated, thresh, kernel, new cv.Point(-1, -1), 1);
...
// v1.1: Hilfsfunktion für adaptive Thresholds
function calculateMedian(mat) {
    const data = [];
    const dataArray = mat.data;
